...
input = open("haiku_lines.txt").read().lower()
...
# snapshot_step=5000, n_epoch=10, batch_size = 128)
# model.save(filename)
...
for _ in range(5):
 ...
 print ('generated:')
 for _ in range(100):
	 print((intchar[np.argmax(model.predict((np.reshape(p, (1, len(p), 1))/float(len(chars)))))]), end='')
